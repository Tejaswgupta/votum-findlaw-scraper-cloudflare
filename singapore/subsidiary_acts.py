...
if not supabase_url or not supabase_key:
    logging.error("Supabase URL or Key not found. Please set SUPABASE_URL and SUPABASE_SERVICE_KEY environment variables.")
...
def get_existing_act(source_id: str, supabase_client: Client) -> dict | None:
    ...
    try:
        ...
        response = supabase_client.table('acts').select('act_id, source_id').eq('source_id', source_id).limit(1).execute()
        ...
        logging.error(f"Error checking for existing act with source_id '{source_id}': {e}")
        return None
...
def store_in_supabase(act_data: dict, all_sections_data: list, supabase_client: Client) -> bool:
    """
    ...
    if not sl_source_id:
        logging.error("SL data is missing 'source_id'. Cannot proceed with Supabase operation.")
        return False
    ...
    if existing_sl:
        ...
        logging.info(f"SL '{sl_name_for_logs}' (source_id: {sl_source_id}) already exists with act_id: {inserted_act_id}.")
        ...
        try:
            ...
            sl_to_insert = {
                ...
                if v is not None or k in ['act_description', 'source', 'source_id', 'parent_id']
            }
            ...
            sl_to_insert['parent_id'] = sl_to_insert.get('parent_id') # Explicitly set to None if not present

            logging.info(f"Attempting to insert SL: {sl_name_for_logs}")
            logging.debug(f"SL data for insertion: {json.dumps(sl_to_insert, indent=2)}")
            sl_insert_response = supabase_client.table('acts').insert(sl_to_insert).execute()

            if hasattr(sl_insert_response, 'data') and sl_insert_response.data:
                inserted_act_id = sl_insert_response.data[0]['act_id']
                logging.info(f"Successfully inserted SL '{sl_name_for_logs}', got act_id: {inserted_act_id}")
            else:
                ...
                logging.error(f"Error inserting SL '{sl_name_for_logs}': {error_message}")
                logging.debug(f"Full SL Insert Response: {sl_insert_response}")
                # If parent_id constraint fails, it might show up here
                if 'violates foreign key constraint' in error_message and 'parent_id' in error_message:
                     logging.error(f"Potential issue: The parent_id ({act_data.get('parent_id')}) might not exist in the acts table.")
                return False

        except Exception as e:
            logging.error(f"An exception occurred during SL insertion for '{sl_name_for_logs}': {e}")
            return False

    # 3. Handle Sections (Insert new ones)
    if not inserted_act_id:
        logging.error(f"Cannot insert sections as act_id was not obtained for '{sl_name_for_logs}'.")
        return False
    ...
    logging.info(f"Found {len(existing_section_titles)} existing sections for SL act_id {inserted_act_id}.")
    ...
    if not new_sections_data:
        logging.info(f"No new sections found to insert for SL act_id {inserted_act_id}.")
        return True # SL exists or was inserted, no new sections needed

    logging.info(f"Found {len(new_sections_data)} new sections to insert for SL act_id {inserted_act_id}.")
    ...
    for section in new_sections_data:
        ...
        if not all(key in section_prepared for key in ['section_title', 'section_content', 'act_id', 'country']):
            logging.warning(f"Skipping section due to missing required fields: {section_prepared.get('section_title', 'NO TITLE')} for SL act_id {inserted_act_id}")
            continue

        sections_to_insert.append(section_prepared)

    if not sections_to_insert:
        logging.warning(f"All {len(new_sections_data)} potential new sections were filtered out due to missing data for SL act_id {inserted_act_id}.")
        return True # SL part was successful

    # Batch insert sections
    try:
        logging.info(f"Attempting to insert {len(sections_to_insert)} new sections linked to SL act_id {inserted_act_id}...")
        ...
        for i in range(0, len(sections_to_insert), batch_size):
            ...
            logging.info(f"  Inserting section batch {i // batch_size + 1} ({len(batch)} sections)...")
            ...
            if not (hasattr(sections_insert_response, 'data') and sections_insert_response.data):
                ...
                logging.error(f"Error inserting Sections for SL act_id {inserted_act_id}: {error_message}")
                logging.debug(f"Full Section Insert Response: {sections_insert_response}")
                all_sections_inserted = False
                break # Stop inserting further batches on error

        if all_sections_inserted:
            logging.info(f"Successfully inserted all {len(sections_to_insert)} new sections for SL act_id {inserted_act_id}.")
            return True
        else:
            logging.error(f"Section insertion failed for one or more batches for SL act_id {inserted_act_id}.")
            return False # Indicate partial failure

    except Exception as e:
        logging.error(f"An exception occurred during Section insertion for SL act_id {inserted_act_id}: {e}")
        return False
...
def extract_sections_from_html(html_content: str, sl_path_for_logs: str) -> list:
    ...
    sections = []
    if not html_content:
        logging.warning(f"[{sl_path_for_logs}] No HTML content provided to extract_sections_from_html.")
        return sections
    ...
    try:
        ...
        if not sections_container:
            logging.warning(f"[{sl_path_for_logs}] Could not find 'div#legisContent'. Trying body.")
            sections_container = soup.find('div', class_='body-content') # Broader fallback
            if not sections_container:
                logging.warning(f"[{sl_path_for_logs}] Could not find content container. Trying entire soup.")
        ...
        if not section_divs and not schedule_divs:
             logging.warning(f"[{sl_path_for_logs}] No 'div.provX' or 'div.schedule' elements found within the content.")
             return sections
        ...
        for element in all_prov_elements:
            ...
            if is_prov:
                header_tag = element.find(['td', 'div'], class_=re.compile(r'prov\d+Hdr'))
                content_tag = element.find(['td', 'div'], class_=re.compile(r'prov\d+Txt'))
                if not content_tag: content_tag = element # Fallback if no specific Txt tag
                if header_tag: title_text_only = header_tag.get_text(strip=True)
                if header_tag: header_id = header_tag.get('id')

            elif is_schedule:
                header_tag = element.find(['td', 'div', 'p'], class_=re.compile(r'(sHdr|scHdr)'))
            ...
            else: continue # Skip if neither prov nor schedule

            if not content_tag:
                logging.warning(f"[{sl_path_for_logs}] Could not find content tag for element: {title_text_only[:50]}...")
                continue
            ...
            if number_tag:
                ...
                if re.match(r'^\d+[A-Z]?\.?$', potential_number): # Match "1." or "2A." etc.
                    section_number_text = potential_number
                    # Clean title: remove number if present, then format
                    clean_title = re.sub(r'^' + re.escape(potential_number) + r'\s*', '', full_title).strip()
                    full_title = f"Rule {section_number_text} {clean_title}".strip() # Use "Rule" for SL? Or keep generic? Let's try Rule.
            ...
            if not section_number_text and header_id:
                 ...
                 if match_prov:
                     ...
                     full_title = f"Rule {section_number_text} {title_text_only}" # Use Rule
                 else:
            ...
            if not section['section_title']: section['section_title'] = "Untitled Section/Schedule" # Fallback title


            # --- Content Extraction ---
            section_content_raw = content_tag.get_text(separator='\n', strip=True)
            processed_content = re.sub(r'\n{3,}', r'\n\n', section_content_raw) # Clean excessive newlines
            ...
            if section_number_text:
                ...
                processed_content = re.sub(r'^' + escaped_num + r'[\s\n]*', '', processed_content, count=1).strip()
        ...
        logging.error(f"[{sl_path_for_logs}] Error during HTML parsing in extract_sections_from_html: {e}", exc_info=True)

    return sections


# --- Main Scraping Logic for SL ---
def scrape_subsidiary_legislation(sl_path: str, session: requests.Session) -> tuple[dict | None, list | None]:
    """
    ...
    if not sl_path or not sl_path.startswith('/SL/'):
        logging.error(f"Invalid path provided to scrape_subsidiary_legislation: '{sl_path}'. Path must start with /SL/.")
        return None, None
    ...
    try:
        ...
        response_initial = session.get(target_url, headers=headers, timeout=20) # Added timeout
        ...
        if not title_tag:
             ...
             if title_div: title_tag = title_div.find('span') # Try span inside legis-title
        sl_data['act_name'] = title_tag.get_text(strip=True) if title_tag else f"UNKNOWN SL ({sl_data['source_id']})"
        sl_data['act_description'] = "" # SL usually doesn't have a separate long description field like Acts

        # Find Parent Act Link and ID
        # NOTE: Parent Act MUST exist in the database first for the foreign key constraint
        parent_act_link = soup_initial.find('a', string=re.compile("Authorising Act", re.IGNORECASE), href=True)
        if parent_act_link:
            ...
            if parent_act_path and parent_act_path.startswith('/Act/'):
                ...
                logging.info(f"[{sl_path}] Found parent Act link: {parent_act_path} (Source ID: {parent_source_id})")
                ...
                if parent_act_id:
                    ...
                    logging.warning(f"[{sl_path}] Parent Act with source_id '{parent_source_id}' not found in DB. 'parent_id' will be NULL.")
                    sl_data['parent_id'] = None # Explicitly set to None
            else:
                 logging.warning(f"[{sl_path}] Found 'Authorising Act' link, but href '{parent_act_path}' is not a valid Act path.")
        else:
            logging.warning(f"[{sl_path}] Could not find 'Authorising Act' link. 'parent_id' will be NULL.")
        ...
        logging.info(f"--- Total Sections Extracted for {sl_path}: {len(all_sections_data)} ---")

        if all_sections_data:
            # Sort sections (optional but good practice)
            def sort_key(section):
                title = section.get('section_title', '')
                match = re.search(r'(?:Rule|Section|SCHEDULE)\s*(\d+)([A-Z]*)', title, re.IGNORECASE)
                if match:
                    num_part = int(match.group(1))
                    alpha_part = match.group(2).upper()
                    alpha_val = sum((ord(char) - ord('A') + 1) * (100 ** i) for i, char in enumerate(reversed(alpha_part)))
    ...
    except (AttributeError, TypeError, ValueError) as e: # Catch parsing/extraction errors
        logging.error(f"[{sl_path}] Data extraction or processing error: {e}", exc_info=True)
    except Exception as e:
...
def scrape_and_store_multiple_sls(sl_paths_to_scrape: list):
    ...
    for index, sl_path in enumerate(sl_paths_to_scrape, 1):
        ...
        try:
            ...
            if sl_data and all_sections_data is not None: # Check sections is not None (can be empty list)
                sl_name_log = sl_data.get('act_name', f"Unknown ({sl_path})")
                logging.info(f"Successfully scraped SL '{sl_name_log}' with {len(all_sections_data)} sections.")
                ...
                if success:
                    logging.info(f"Successfully stored/updated SL '{sl_name_log}' in Supabase.")
                    results[sl_path] = True
                else:
                    logging.error(f"Failed to store/update SL '{sl_name_log}' in Supabase.")
                    results[sl_path] = False
            elif sl_data and all_sections_data is None:
                 logging.error(f"Scraped metadata for SL '{sl_path}' but failed to extract sections.")
                 results[sl_path] = False
            else:
            ...
            logging.error(f"Critical error processing SL path {sl_path}: {e}", exc_info=True)
            results[sl_path] = False
...
def get_all_sl_paths(session: requests.Session) -> list:
    ...
    while True:
        ...
        try:
            headers = base_headers.copy()
            headers['referer'] = "https://sso.agc.gov.sg/Browse/SL/Current/All" # Appropriate referer
            response = session.get(url, headers=headers, timeout=30)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, 'lxml')
            # Find links specifically starting with /SL/ within the results area
            browse_results = soup.find('div', class_='browse-list-row') # Target results area
            links_found = []
            if browse_results:
                links_found = browse_results.find_all('a', href=lambda href: href and href.startswith("/SL/"))
            else:
                 logging.warning(f"Could not find 'div.browseResults' on page {index}. Trying whole page.")
                 links_found = soup.find_all('a', href=lambda href: href and href.startswith("/SL/"))


            if not links_found:
                 logging.info(f"No more '/SL/' links found on page index {index}. Stopping.")
                 break
            ...
            logging.info(f"Found {count_on_page} new SL paths on index {index}. Total unique paths: {len(sl_paths)}")
            ...
            if len(count_on_page) < page_size:
                logging.info(f"Found {len(links_found)} links (less than PageSize={page_size}), assuming end of list.")
                break

            index += 1 # Move to the next page index

        except requests.exceptions.Timeout:
             logging.error(f"Timeout fetching SL browse page {url}. Stopping path collection.")
             break
        except requests.exceptions.RequestException as e:
            logging.error(f"Failed to fetch SL browse page {url}: {e}. Stopping path collection.")
            break
        except Exception as e:
            logging.error(f"Error parsing SL browse page {url}: {e}. Stopping path collection.")
            break

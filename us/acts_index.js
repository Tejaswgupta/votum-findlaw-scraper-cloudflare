...
async function getExistingActId(actName) {
	try {
		const { data, error } = await supabase.from('acts').select('act_id').eq('act_name', actName).single();
		if (error) {
...
async function insertAct(actName) {
	try {
		...
		const { data, error } = await supabase
			.from('acts')
			.insert([{ act_name: actName, country: 'US' }])
			.select()
			.single();
...
async function checkIfProcessed(siteMapUrl) {
	const { data, error } = await supabase.from('acts_sections_scraping_sitemap').select('*').eq('url', siteMapUrl).eq('processed', true);
	if (error) {
...
async function upsertSiteMap(siteMapUrl) {
	...
	const { data, error } = await supabase
		...
		.insert([{ url: siteMapUrl, processed: true, created_at: new Date() }]);
